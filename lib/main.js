...
export default {
    ...
    writeConfig(options) {
        var accentColor = atom.config.get('papyrus-ui.colors.accentColor').toHexString();
        var baseColor = atom.config.get('papyrus-ui.colors.abaseColor').toHexString();
        var accentTextColor = this.getContrast(baseColor);
        var fontSize = atom.config.get('papyrus-ui.fonts.fontSize');
        ...
        */
        ...
        fs.writeFile(`${__dirname}/../styles/custom.less`, config, 'utf8', () => {
            if (!options || !options.noReload) {
                var themePack = atom.packages.getLoadedPackage('papyrus-ui');
